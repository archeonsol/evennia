...
class AuthorizationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.account = get_user_model().objects.create(username="grantee", is_active=True)
        grant_capability(
    ...
    def io(self):
        ...
        return IOContext(actor_id=self.account.pk, actor_name="grantee", capabilities=frozenset())
...
class TestViews(AuthorizationTestCase):
    ...
    def test_filtering_by_principal(self):
        ...
        self.assertFalse(self.panel.rows(_ctx(view="grants", principal="account:99999"))["rows"])

    def test_filtering_by_capability(self):
        self.assertTrue(self.panel.rows(_ctx(view="grants", capability=CAPABILITY))["rows"])
        self.assertFalse(
            self.panel.rows(_ctx(view="grants", capability="engine.object.view"))["rows"]
        )
    ...
    def test_bundles_are_expanded(self):
        names = {name for name, _ in self.panel.rows(_ctx())["bundles"]}
...
class TestProbe(AuthorizationTestCase):
    ...
    def test_a_held_capability_is_allowed(self):
        result = self.panel.probe(self.io(), principal_id=self.account.pk, capability=CAPABILITY)
    ...
    def test_the_grant_scope_is_reported(self):
        result = self.panel.probe(self.io(), principal_id=self.account.pk, capability=CAPABILITY)
        scopes = {(row["scope_kind"], row["scope_key"]) for row in result["matching_grants"]}
    ...
    def test_an_unknown_capability_is_a_lookup_error(self):
        with self.assertRaises(LookupError):
            self.panel.probe(self.io(), principal_id=self.account.pk, capability="not.a.capability")
...
class TestBreakGlass(AuthorizationTestCase):
    ...
    def test_it_is_audited_permanently(self):
        ...
        # want gone.
        self.panel.break_glass(self.io(), principal_id=self.account.pk, reason="incident 4")
        row = ConsoleAuditEvent.objects.get(panel="authorization", operation="break_glass_issue")
    ...
    def test_it_says_what_it_does_not_bypass(self):
        result = self.panel.break_glass(self.io(), principal_id=self.account.pk, reason="r")
