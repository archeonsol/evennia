...
class TestLauncher(TwistedTestCase):
    ...
    def test_module_preflight_has_no_retired_lock_setting(self):
        source = inspect.getsource(evennia_launcher.error_check_python_modules)
        self.assertNotIn("LOCK_FUNC_MODULES", source)
...
class TestForceKillIdentity(unittest.TestCase):
    ...
    def test_our_process_matches_by_cmdline(self, mock_proc):
        ...
        mock_proc.return_value.cmdline.return_value = [
            "/py",
            "/lib/server.py",
            "--pidfile",
            "x",
    ...
    def test_force_kill_terminates_our_process(self, _mock_ipc):
        ...
        with (
            ...
            patch(
                "evennia.server.evennia_launcher.psutil.wait_procs",
                return_value=([proc], []),
            ),
        ):
            evennia_launcher._force_kill_local_processes()
