<script lang="ts">
  ...
  // Whatever is on screen has been seen. A move-route's lines are nowhere else,
  // so the badge on the other tabs is the only notice the player gets.
  $effect(() => {
    void lines.length;
    if (active) routing.markRead(active);
  });
</script>
