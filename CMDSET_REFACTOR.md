...
3. **Engine default commands keyed for the new world.**
   Most builder commands (`@open`, `@dig`, etc.) are already correctly
   keyed in the engine. The actual re-key work is heavier on the
   admin/account side, which today ships staff-only commands at the
   unprefixed player namespace (`ban`, `emit`, `force`, `charcreate`,
   `quell`, etc.). The convention is documented in
   [`.agents/docs/code-style.md`](.agents/docs/code-style.md) (Command Naming):
   `@` for player/account/OOC actions, no prefix for character/IC actions.

   The frozen inventory of current keys lives at
   [`PHASE3_AUDIT.md`](PHASE3_AUDIT.md), regenerated by
   `.agents/tools/cmdset_prefix_audit.py`. CI runs the audit in `--check`
   mode (`TestCmdsetPrefixAuditDrift` in `evennia/commands/tests.py`)
   to catch drift mid-Phase-3.

   Re-key list derived from the recon (CharacterCmdSet → needs `@`,
   AccountCmdSet → needs `@`):

   *Admin (`admin.py`):* `ban` → `@ban`, `unban` → `@unban`,
   `boot` → `@boot`, `emit` → `@emit`, `force` → `@force`,
   `perm` → `@perm`, `wall` → `@wall`, `userpassword` → `@userpassword`.

   *Batch (`batchprocess.py`):* `batchcode` → `@batchcode`,
   `batchcommands` → `@batchcommands`.

   *Account (`account.py`):* `charcreate` → `@charcreate`,
   `chardelete` → `@chardelete`, `ic` → `@ic`, `ooc` → `@ooc`,
   `option` → `@option`, `password` → `@password`,
   `quell` → `@quell`, `quit` → `@quit`, `color` → `@color`,
   `style` → `@style`, `who` → `@who`, `sessions` → `@sessions`.
   (`look` in `AccountCmdSet` stays unprefixed by analogy with the
   character `look`; the OOC look just lists characters.)

   *Account staff (`account.py`):* `nick` → `@nick`, `access` → `@access`
   (newmoo treats both as OOC; the engine should too — nicks are player
   input shortcuts, not character speech).

   *Building (`building.py`):* `unlink` → `@unlink` (the lone straggler;
   surrounding `@link`/`@sethome` are correctly prefixed).

   *Help (`help.py`):* `sethelp` → `@sethelp`. `help` itself stays
   unprefixed (meta carve-out, like channels).

   *Comms (`comms.py`):* `irc2chan` → `@irc2chan`,
   `ircstatus` → `@ircstatus`, `rss2chan` → `@rss2chan`,
   `grapevine2chan` → `@grapevine2chan`,
   `discord2chan` → `@discord2chan`. `page` → `@page` (account-level
   messaging, OOC by the rule; downstreams that don't use page are
   unaffected).

   Channel chat commands auto-generated per-channel keep their
   channel-key names (e.g. `ooc`, `public`) — this is the explicit
   carve-out. `CmdChannel` (the channel-manager meta command) is
   already keyed `@channel`. The auto-generated `ooc` channel command
   and the renamed `@ooc` (account-puppet-out) won't collide because
   prefix-strip is gone.

   **UnloggedinCmdSet** is out of scope for re-keying — pre-login
   commands are functional/UI (`connect`, `create`, `quit`,
   `screenreader`) and the IC/OOC distinction doesn't apply since
   there's no character yet.

   **Before merging Phase 3:** confirm the re-key list against
   `PHASE3_AUDIT.md` (and against newmoo's wrapper inventory, since
   newmoo's `CmdAt*` wrappers are the ground truth for what downstream
   wanted re-keyed). The drift test in CI prevents the audit from
   going stale during the Phase 3 branch's lifetime.
